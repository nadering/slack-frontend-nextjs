...
import { getWorkspace, inviteToWorkspace } from "@/apis";
import {
  showInviteToWorkspaceModalState,
  workspaceInformationState,
} from "@/store";

// 워크스페이스 초대에 사용되는 전체 화면을 덮는 모달
export default function InviteToWorkspaceModal() {
  ...
  async function handleInviteToWorkspace() {
    const inviteToWorkspaceData = {
      workspaceId: currentWorkspaceId,
      targetUserEmail: targetUserEmail,
    };
    const [inviteToWorkspaceSuccess, userList] = await inviteToWorkspace(
      inviteToWorkspaceData
    );

    // 채널 생성에 성공하면 워크스페이스 정보를 새로 받아와 저장합니다.
    if (inviteToWorkspaceSuccess) {
    ...
    <div className="layer pack z(100) bg(--fullscreen-modal-dark-background)">
      <div className="relative w(520) bg(--fullscreen-modal-background) r(8) p(24/28) font-family(Larsseit) @w(~585):w(324)">
        ...
        <button
          ...
          onClick={() => setShowInviteToWorkspaceModal(false)}
        >
          <div className="pack">
            <Image
